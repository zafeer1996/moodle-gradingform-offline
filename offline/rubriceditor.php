...
class MoodleQuickForm_rubriceditor extends HTML_QuickForm_input {
     ...
     *
     * @see gradingform_offline_controller::update_or_check_rubric()
    ...
    public function toHtml() {
        global $PAGE;
        $html = $this->_getTabs();
        $renderer = $PAGE->get_renderer('gradingform_offline');
        $data = $this->prepare_data(null, $this->wasvalidated);



        if (!$this->_flagFrozen) {
            $mode = gradingform_offline_controller::DISPLAY_EDIT_FULL;
            $module = array('name'=>'gradingform_offlineeditor', 'fullpath'=>'/grade/grading/form/offline/js/rubriceditor.js',
                'requires' => array('base', 'dom', 'event', 'event-touch', 'escape'),
                'strings' => array(array('confirmdeletecriterion', 'gradingform_offline'), array('confirmdeletelevel', 'gradingform_offline'),
                    array('criterionempty', 'gradingform_offline'), array('levelempty', 'gradingform_offline')
                ));
            $PAGE->requires->js_init_call('M.gradingform_offlineeditor.init', array(
    ...
    protected function prepare_data($value = null, $withvalidation = false) {
        ...
        $totalscore = 0;
        $errors = array();
        $return = array('criteria' => array(), 'options' => gradingform_offline_controller::get_default_options());
        ...
        foreach ($value['criteria'] as $id => $criterion) { //iterates through criterion already on page
            if ($id == 'addcriterion') {
        ...
        if ($withvalidation) {
            ...
            if (count($errors)) {
                ...
                foreach ($errors as $error => $v) {
                    $rv[] = get_string($error, 'gradingform_offline');
                }
