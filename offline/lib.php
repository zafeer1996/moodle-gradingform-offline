...
/**
 * Grading method controller for the Offline plugin
 *
 * @package    gradingform_offline
 * @copyright  2020 Zafeer Mohammed <zafeermohammed19@yahoo.com>
...
/** offline: Used to compare our gradeitem_type against. */
const OFFLINE = 'offline';

/**
 * This controller encapsulates the offline grading logic
 *
 * @package    gradingform_offline
 * @copyright  2020 Zafeer Mohammed <zafeermohammed19@yahoo.com>
 * @copyright  based on work by 2011 David Mudrak <david@moodle.com>
...
class gradingform_offline_controller extends gradingform_controller {
    // Modes of displaying the rubric (used in gradingform_offline_renderer)
    ...
    /**
     * Extends the module settings navigation with the offline grading settings
     *
     * This function is called when the context for the page is an activity module with the
     * FEATURE_ADVANCED_GRADING, the user has the permission moodle/grade:managegradingforms
     * and there is an area with the active grading method set to 'offline'.
     *
    ...
    public function extend_settings_navigation(settings_navigation $settingsnav, navigation_node $node=null) {
        $node->add(get_string('definerubric', 'gradingform_offline'),
            $this->get_editor_url(), settings_navigation::TYPE_CUSTOM,
            null, null, new pix_icon('icon', '', 'gradingform_offline'));
    }
    ...
    public function extend_navigation(global_navigation $navigation, navigation_node $node=null) {
        ...
        if ($this->is_form_defined() && ($options = $this->get_options()) && !empty($options['alwaysshowdefinition'])) {
            $node->add(get_string('gradingof', 'gradingform_offline', get_grading_manager($this->get_areaid())->get_area_title()),
     ...
     * @param stdClass $newdefinition offline definition data as coming from gradingform_offline_editrubric::get_data()
     * @param int|null $usermodified optional userid of the author of the definition, defaults to the current user
     */
    public function update_definition(stdClass $newdefinition, $usermodified = null) {
        $this->update_or_check_offline($newdefinition, $usermodified, true);
     ...
     *
     * @param stdClass $newdefinition rubric definition data as coming from gradingform_offline_editrubric::get_data()
     * @param int|null $usermodified optional userid of the author of the definition, defaults to the current user
     * @param boolean $doupdate if true actually updates DB, otherwise performs a check
     *
     */
    public function update_or_check_offline(stdClass $newdefinition, $usermodified = null, $doupdate = false) {
        global $DB;
        ...
        foreach ($newcriteria as $id => $criterion) {
            ...
            if (preg_match('/^NEWID\d+$/', $id)) {
                ...
                if ($doupdate) {
                    $id = $DB->insert_record('gradingform_offline_criteria', $data);
                }
                $haschanges[5] = true;
            } else {
                ...
                if (!empty($data)) {
                    ...
                    $data['id'] = $id;
                    if ($doupdate) {
                        $DB->update_record('gradingform_offline_criteria', $data);
                    }
                    $haschanges[1] = true;
                }
                ...
                foreach ($currentcriteria[$id]['levels'] as $levelid => $currentlevel) {
                    ...
                    if (!array_key_exists($levelid, $levelsdata)) {
                        if ($doupdate) {
                            $DB->delete_records('gradingform_offline_levels', array('id' => $levelid));
                        }
                        $haschanges[4] = true;
                    }
                }
            ...
            foreach ($levelsdata as $levelid => $level) {
                ...
                if (preg_match('/^NEWID\d+$/', $levelid)) {
                    ...
                    if ($doupdate) {
                        $levelid = $DB->insert_record('gradingform_offline_levels', $data);
                    }
                    ...
                    if (!empty($data)) {
                        ...
                        $data['id'] = $levelid;
                        if ($doupdate) {
                            $DB->update_record('gradingform_offline_levels', $data);
                        }
                        if (isset($data['score'])) {
                            $haschanges[3] = true;
                        }
        ...
        foreach (array_keys($currentcriteria) as $id) {
            if (!array_key_exists($id, $newcriteria)) {
                if ($doupdate) {
                    $DB->delete_records('gradingform_offline_criteria', array('id' => $id));
                    $DB->delete_records('gradingform_offline_levels', array('criterionid' => $id));
                }
                $haschanges[3] = true;
            }
        }
    ...
    protected function load_definition() {
        ...
        $sql = "SELECT gd.*,
             ...
             LEFT JOIN {gradingform_offline_criteria} rc ON (rc.definitionid = gd.id)
             LEFT JOIN {gradingform_offline_levels} rl ON (rl.criterionid = rc.id)
     ...
     * @return gradingform_offline_renderer
     */
    ...
    public function render_preview(moodle_page $page) {
        ...
        if ($showdescription) {
            $rubric .= $output->box($this->get_formatted_description(), 'gradingform_offline-description');
        }
        if (has_capability('moodle/grade:managegradingforms', $page->context)) {
    ...
    protected function delete_plugin_definition() {
        ...
        // delete all fillings
        $DB->delete_records_list('gradingform_offline_fillings', 'instanceid', $instances);
        // delete instances
        $DB->delete_records_list('grading_instances', 'id', $instances);
        // get the list of criteria records
        $criteria = array_keys($DB->get_records('gradingform_offline_criteria', array('definitionid' => $this->definition->id), '', 'id'));
        // delete levels
        $DB->delete_records_list('gradingform_offline_levels', 'criterionid', $criteria);
        // delete critera
        $DB->delete_records_list('gradingform_offline_criteria', 'id', $criteria);
    }
    ...
    public function get_or_create_instance($instanceid, $raterid, $itemid) {
        ...
        if ($itemid && $raterid) {
            ...
            if ($rs = $DB->get_records('grading_instances', $params, 'timemodified DESC', '*', 0, 1)) {
                ...
                if ($record->status == gradingform_offline_instance::INSTANCE_STATUS_INCOMPLETE &&
    ...
    public static function sql_search_from_tables($gdid) {
        return " LEFT JOIN {gradingform_offline_criteria} rc ON (rc.definitionid = $gdid)
                 LEFT JOIN {gradingform_offline_levels} rl ON (rl.criterionid = rc.id)";
    }
    ...
    /**
     * Returns the altered data structure where $content is appended to $data->rubric['criteria'].
     * A criterion array ($critarray) is constructed and populated with the criteria, levels and fields from the $content array.
     * The title of the form is set to the name of the uploaded file and the description of the form is set
     * to the first name and last name of the user who uploaded the file.
     *
     * @param string $name name of the uploaded file
     * @param object $data data collected from the mform
     * @param array $content array of rows of data from the file uploaded by user
     * @param string $redirect contains the url to redirect the page to the form in case of improper file format submitted
     * @return object containing the appended data
     */
    public function import_data($name,$data,$content,$redirect) {

        global $USER;
        $return = $data;
        $error = get_string('noerrors','gradingform_offline');
        $criterrors = '';
        $lvlcount = 0;
        $critarray = array();

        //creating and populating array "$critarray" to hold criterion:

        //check that content has an even number of rows else exit with error
        $rowcount = count($content);
        if ($rowcount%2 != 0) {
            //redirect to form showing warning of improper file format
            $warning = get_string('unevenrows', 'gradingform_offline');
            redirect($redirect, $warning, null, \core\output\notification::NOTIFY_ERROR);
        }
        else{
        //iterate through content array by 2
            for ($i=0;$i<$rowcount/2;$i++) {
                //initialize lvl counters
                $lvlcount = 0;
                //initialize arrays
                $criterion = array('sortorder'=>'','description'=>'', 'levels'=>array());
                $lvl = array('definition'=>'', 'score'=>'');
                //pull criteria row from content and put it in $critrow (2*$i)
                $critrow = str_getcsv($content[2*$i]);
                //pull score row from content and put it in $scorerow (2*$i + 1)
                $scorerow = str_getcsv($content[2*$i + 1]);

                //$criterion['description'] set to  second element of $critrow array 
                $criterion['description'] = $critrow[1];

                //iterate through the rows starting from 2 til end of row
                for ($a=2;$a<count($critrow);$a++) {
                    //if either level element or score element is null, do not set either
                    if ($critrow[$a] != null && $scorerow[$a] != null){
                        //$lvl['definition'] would come from $critrow[$a]
                        $lvl['definition'] = $critrow[$a];
                        //$lvl['score'] would come from $scorerow[$a
                        $lvl['score'] = $scorerow[$a];

                        //set criterion['levels'] to $lvl
                        $criterion['levels']['NEWID'.$lvlcount] = $lvl;
                        $lvlcount++;
                    }
                    //setting error if either of level or score field is blank but not if both (for criteria with less levels than others)
                    else if ($critrow[$a] == null xor $scorerow[$a] == null){
                        $error = get_string('nullfields', 'gradingform_offline');
                        $criterrors .= "\n " . $critrow[1];
                    }
                }
                $critarray['NEWID'.$i] = $criterion;

            }
        }

        if ($error == get_string('nullfields', 'gradingform_offline')){
            $error .= $criterrors;
        }
        //setting the criteria to the data structure
        $return->rubric['criteria'] = $critarray;
        //setting the name of the form to the name of the file
        $return->name = $name;
        //setting the description of the form to the User's name as well as any errors detected while extracting data
        $return->description_editor['text'] = get_string('importedby', 'gradingform_offline') . " " . $USER->firstname . " " . $USER->lastname . ". " . $error;
        $return->description_editor['format'] = '';
        return $return;
    }
}
 ...
 * @package    gradingform_offline
 * @copyright  2020 Zafeer Mohammed <zafeermohammed19@yahoo.com>
 * @copyright  based on work by 2011 Marina Glancy
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class gradingform_offline_instance extends gradingform_instance {
    ...
    public function cancel() {
        global $DB;
        parent::cancel();
        $DB->delete_records('gradingform_offline_fillings', array('instanceid' => $this->get_id()));
    }
    ...
    public function copy($raterid, $itemid) {
        ...
        foreach ($currentgrade['criteria'] as $criterionid => $record) {
            ...
            $DB->insert_record('gradingform_offline_fillings', $params);
        }
        return $instanceid;
    }
    ...
    public function clear_attempt($data) {
        ...
        foreach ($data['criteria'] as $criterionid => $record) {
            $DB->delete_records('gradingform_offline_fillings',
    ...
    public function get_rubric_filling($force = false) {
        ...
        if ($this->rubric === null || $force) {
            $records = $DB->get_records('gradingform_offline_fillings', array('instanceid' => $this->get_id()));
    ...
    public function update($data) {
        ...
        foreach ($data['criteria'] as $criterionid => $record) {
            if (!array_key_exists($criterionid, $currentgrade['criteria'])) {
                ...
                $DB->insert_record('gradingform_offline_fillings', $newrecord);
            } else {
                ...
                if (count($newrecord) > 1) {
                    $DB->update_record('gradingform_offline_fillings', $newrecord);
                }
            }
        }
        foreach ($currentgrade['criteria'] as $criterionid => $record) {
            if (!array_key_exists($criterionid, $data['criteria'])) {
                $DB->delete_records('gradingform_offline_fillings', array('id' => $record['id']));
            }
        }
    ...
    public function render_grading_element($page, $gradingformelement) {
        ...
        if (!$gradingformelement->_flagFrozen) {
            $module = array('name'=>'gradingform_offline', 'fullpath'=>'/grade/grading/form/offline/js/rubric.js');
            $page->requires->js_init_call('M.gradingform_offline.init', array(array('name' => $gradingformelement->getName())), true, $module);
            $mode = gradingform_offline_controller::DISPLAY_EVAL;
        } else {
            if ($gradingformelement->_persistantFreeze) {
                $mode = gradingform_offline_controller::DISPLAY_EVAL_FROZEN;
            } else {
                $mode = gradingform_offline_controller::DISPLAY_REVIEW;
            }
        }
        $criteria = $this->get_controller()->get_definition()->rubric_criteria;
        $options = $this->get_controller()->get_options();
        ...
        if ($value === null) {
            ...
            $html .= html_writer::tag('div', get_string('rubricnotcompleted', 'gradingform_offline'), array('class' => 'gradingform_offline-error'));
        }
        $currentinstance = $this->get_current_instance();
        if ($currentinstance && $currentinstance->get_status() == gradingform_instance::INSTANCE_STATUS_NEEDUPDATE) {
            $html .= html_writer::div(get_string('needregrademessage', 'gradingform_offline'), 'gradingform_offline-regrade',
                                      array('role' => 'alert'));
        }
        $haschanges = false;
        if ($currentinstance) {
        ...
        if ($this->get_data('isrestored') && $haschanges) {
            $html .= html_writer::tag('div', get_string('restoredfromdraft', 'gradingform_offline'), array('class' => 'gradingform_offline-restored'));
        }
        if (!empty($options['showdescriptionteacher'])) {
            $html .= html_writer::tag('div', $this->get_controller()->get_formatted_description(), array('class' => 'gradingform_offline-description'));
        }
