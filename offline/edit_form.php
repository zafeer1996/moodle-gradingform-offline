 ...
 * @copyright  based on work by 2011 Marina Glancy <marina@moodle.com>
...
MoodleQuickForm::registerElementType('rubriceditor', $CFG->dirroot.'/grade/grading/form/offline/rubriceditor.php', 'MoodleQuickForm_rubriceditor');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class gradingform_offline_editoffline extends moodleform {
    ...
    public function definition() {
        ...
        //accepted file types for filepicker element
        $acceptedtypes = '.csv';
        $form = $this->_form;
        ...
        // name
        $form->addElement('text', 'name', get_string('name', 'gradingform_offline'), array('size' => 52, 'aria-required' => 'true'));
        $form->addRule('name', get_string('required'), 'required', null, 'client');
        $form->setType('name', PARAM_TEXT);

        // description
        $options = gradingform_offline_controller::description_form_field_options($this->_customdata['context']);
        $form->addElement('editor', 'description_editor', get_string('description', 'gradingform_offline'), null, $options);
        ...
        $form->addElement('select', 'status', get_string('rubricstatus', 'gradingform_offline'), $choices)->freeze();

        // rubric editor
        $element = $form->addElement('rubriceditor', 'rubric', get_string('rubric', 'gradingform_offline'));
        $form->setType('offline', PARAM_RAW);

        // filepicker element
        $form->addElement('filepicker', 'userfile',get_string('import', 'gradingform_offline'), null, array('accepted_types' => $acceptedtypes));

        // html link to download template for creating a rubric offline
        $template = '<a href="TEMPLATE.xlsx" download><img src="text-96.png" alt="RubricCSV" width="104" height="104"></a>';
        $form->addElement('static', 'template_link', get_string('template_link', 'gradingform_offline'), $template);

        // non-submit button created and registered
        $form->registerNoSubmitButton('imp');
        $importbutton = &$form->createElement('submit', 'imp', "Import Data");



        $buttonarray = array();
        $buttonarray[] = &$importbutton;
        $buttonarray[] = &$form->createElement('submit', 'saverubric', get_string('saverubric', 'gradingform_offline'));
        if ($this->_customdata['allowdraft']) {
            $buttonarray[] = &$form->createElement('submit', 'saverubricdraft', get_string('saverubricdraft', 'gradingform_offline'));
        }
    ...
    public function definition_after_data() {
        ...
        if (!$el->getValue()) {
            ...
            if ($vals[0] == gradingform_controller::DEFINITION_STATUS_READY) {
                $this->findButton('saverubric')->setValue(get_string('save', 'gradingform_offline'));
            }
        }
    }
     ...
     *
     * @param gradingform_offline_controller $controller
     */
    public function need_confirm_regrading($controller) {
        ...
        $changelevel = $controller->update_or_check_offline($data);
        if ($changelevel == 0) {
        ...
        $el->setValue(get_string('backtoediting', 'gradingform_offline'));
        $el->unfreeze();
    ...
    /**
     * Get the contents of an uploaded file without form being submitted and without conducting validation checks
     *
     * @param string $elname name of file upload element
     * @return string|bool false in case of failure, string if ok
     */
    public function get_contents($elname)
    {
        global $USER;
    ...
    /**
     * Returns name of uploaded file without form being submitted and without conducting validation checks
     *
     * @param string $elname first element if null
     * @return string|bool false in case of failure, string if ok
     */
    function get_filename($elname=null) {
        global $USER;
