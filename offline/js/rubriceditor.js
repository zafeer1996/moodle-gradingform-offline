M.gradingform_offlineeditor = {'templates' : {}, 'eventhandler' : null, 'name' : null, 'Y' : null};

/**
 * This function is called for each rubriceditor on page.
 */
M.gradingform_offlineeditor.init = function(Y, options) {
    M.gradingform_offlineeditor.name = options.name
    M.gradingform_offlineeditor.Y = Y
    M.gradingform_offlineeditor.templates[options.name] = {
        'criterion' : options.criteriontemplate,
        'level' : options.leveltemplate
    }
    M.gradingform_offlineeditor.disablealleditors()
    Y.on('click', M.gradingform_offlineeditor.clickanywhere, 'body', null)
    YUI().use('event-touch', function (Y) {
        Y.one('body').on('touchstart', M.gradingform_offlineeditor.clickanywhere);
        Y.one('body').on('touchend', M.gradingform_offlineeditor.clickanywhere);
    })
    M.gradingform_offlineeditor.addhandlers()
};

// Adds handlers for clicking submit button. This function must be called each time JS adds new elements to html
M.gradingform_offlineeditor.addhandlers = function() {
    var Y = M.gradingform_offlineeditor.Y
    var name = M.gradingform_offlineeditor.name
    if (M.gradingform_offlineeditor.eventhandler) M.gradingform_offlineeditor.eventhandler.detach()
    M.gradingform_offlineeditor.eventhandler = Y.on('click', M.gradingform_offlineeditor.buttonclick, '#rubric-'+name+' input[type=submit]', null);
}

// switches all input text elements to non-edit mode
M.gradingform_offlineeditor.disablealleditors = function() {
    var Y = M.gradingform_offlineeditor.Y
    var name = M.gradingform_offlineeditor.name
    Y.all('#rubric-'+name+' .level').each( function(node) {M.gradingform_offlineeditor.editmode(node, false)} );
    Y.all('#rubric-'+name+' .description').each( function(node) {M.gradingform_offlineeditor.editmode(node, false)} );
}
...
// function is invoked
M.gradingform_offlineeditor.clickanywhere = function(e) {
    ...
    if (el) {
        if (el.one('textarea').hasClass('hiddenelement')) {
            M.gradingform_offlineeditor.disablealleditors()
            M.gradingform_offlineeditor.editmode(el, true, focustb)
        }
        return
    }
...
M.gradingform_offlineeditor.editmode = function(el, editmode, focustb) {
    ...
    if (!taplain) {
        ...
        taplain = ta.get('parentNode').one('.plainvalue')
        taplain.one('.pseudotablink').on('focus', M.gradingform_offlineeditor.clickanywhere)
        if (tb) {
            ...
            tbplain.one('.pseudotablink').on('focus', M.gradingform_offlineeditor.clickanywhere)
        }
    }
    ...
    if (!editmode) {
        ...
        if (value.length) taplain.removeClass('empty')
            ...
            value = (el.hasClass('level')) ? M.util.get_string('levelempty', 'gradingform_offline') : M.util.get_string('criterionempty', 'gradingform_offline')
...
M.gradingform_offlineeditor.buttonclick = function(e, confirmed) {
    var Y = M.gradingform_offlineeditor.Y
    var name = M.gradingform_offlineeditor.name
    ...
    if (action == 'addcriterion' || action == 'addlevel' || action == 'duplicate' ) {
        newid = M.gradingform_offlineeditor.calculatenewid('#rubric-'+name+' .criterion');
        newlevid = M.gradingform_offlineeditor.calculatenewid('#rubric-'+name+' .level');
    }
    var dialog_options = {
        'scope' : this,
        'callbackargs' : [e, true],
        'callback' : M.gradingform_offlineeditor.buttonclick
    };
    if (chunks.length == 3 && action == 'addcriterion') {
        ...
        for (levidx=0;levidx<levelsscores.length;levidx++) {
            levelsstr += M.gradingform_offlineeditor.templates[name].level.
                replace(/\{LEVEL-id\}/g, 'NEWID'+(newlevid+levidx)).
                replace(/\{LEVEL-score\}/g, levelsscores[levidx]).
                replace(/\{LEVEL-index\}/g, levidx + 1);
        }
        var newcriterion = M.gradingform_offlineeditor.templates[name]['criterion'].replace(/\{LEVELS\}/, levelsstr)
        parentel.append(newcriterion.replace(/\{CRITERION-id\}/g, 'NEWID'+newid).replace(/\{.+?\}/g, ''))
        M.gradingform_offlineeditor.assignclasses('#rubric-'+name+' #'+name+'-criteria-NEWID'+newid+'-levels .level')
        M.gradingform_offlineeditor.addhandlers();
        M.gradingform_offlineeditor.disablealleditors()
        M.gradingform_offlineeditor.assignclasses(elements_str)
        M.gradingform_offlineeditor.editmode(
        ...
        var newlevel = M.gradingform_offlineeditor.templates[name]['level'].
        ...
        M.gradingform_offlineeditor.disablealleditors()
        M.gradingform_offlineeditor.assignclasses(elements_str)
        M.gradingform_offlineeditor.editmode(parent.all('.level').item(parent.all('.level').size()-1), true)
        ...
        if (confirmed) {
            ...
            dialog_options['message'] = M.util.get_string('confirmdeletecriterion', 'gradingform_offline')
        ...
        var newcriterion = M.gradingform_offlineeditor.templates[name].criterion
        ...
        M.gradingform_offlineeditor.disablealleditors();
        M.gradingform_offlineeditor.assignclasses(elements_str);
        M.gradingform_offlineeditor.editmode(Y.one('#rubric-'+name+' #'+name+'-criteria-NEWID'+newid+'-description-cell'),true);
        ...
        if (confirmed) {
            ...
            dialog_options['message'] = M.util.get_string('confirmdeletelevel', 'gradingform_offline')
            M.util.show_confirm_dialog(e, dialog_options);
        }
    } else {
        // unknown action
...
M.gradingform_offlineeditor.assignclasses = function (elements_str) {
    var elements = M.gradingform_offlineeditor.Y.all(elements_str)
...
M.gradingform_offlineeditor.calculatenewid = function (elements_str) {
    var newid = 1
    M.gradingform_offlineeditor.Y.all(elements_str).each( function(node) {
