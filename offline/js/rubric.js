M.gradingform_offline = {};

/**
 * This function is called for each rubric on page.
 */
M.gradingform_offline.init = function(Y, options) {
    Y.on('click', M.gradingform_offline.levelclick, '#rubric-'+options.name+' .level', null, Y, options.name);
    // Capture also space and enter keypress.
    Y.on('key', M.gradingform_offline.levelclick, '#rubric-' + options.name + ' .level', 'space', Y, options.name);
    Y.on('key', M.gradingform_offline.levelclick, '#rubric-' + options.name + ' .level', 'enter', Y, options.name);
...
M.gradingform_offline.levelclick = function(e, Y, name) {
    var el = e.target
