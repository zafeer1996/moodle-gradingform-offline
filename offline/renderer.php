 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class gradingform_offline_renderer extends plugin_renderer_base {
     ...
     *
     * @param int $mode rubric display mode, see {@link gradingform_offline_controller}
     * @param array $options display options for this rubric, defaults are: {@link gradingform_offline_controller::get_default_options()}
    ...
    public function criterion_template($mode, $options, $elementname = '{NAME}', $criterion = null, $levelsstr = '{LEVELS}', $value = null) {
        ...
        if ($mode == gradingform_offline_controller::DISPLAY_EDIT_FULL) {
            ...
            $descriptiontextareaparams = array(
                ...
                'aria-label' => get_string('criterion', 'gradingform_offline', ''),
                'cols' => '10', 'rows' => '5'
            );
            $description = html_writer::tag('textarea', s($criterion['description']), $descriptiontextareaparams);
        } else {
            if ($mode == gradingform_offline_controller::DISPLAY_EDIT_FROZEN) {
        ...
        if ($mode != gradingform_offline_controller::DISPLAY_EDIT_FULL &&
            $mode != gradingform_offline_controller::DISPLAY_EDIT_FROZEN) {
            // Set description's cell as tab-focusable.
            $descriptiontdparams['tabindex'] = '0';
            // Set label for the criterion cell.
            $descriptiontdparams['aria-label'] = get_string('criterion', 'gradingform_offline', s($criterion['description']));
        }
        ...
        $levelstableparams = array(
            ...
            'aria-label' => get_string('levelsgroup', 'gradingform_offline')
        );
        ...
        if ($mode == gradingform_offline_controller::DISPLAY_EDIT_FULL) {
            $value = get_string('criterionaddlevel', 'gradingform_offline');
        ...
        $displayremark = ($options['enableremarks'] && ($mode != gradingform_offline_controller::DISPLAY_VIEW || $options['showremarksstudent']));
        if ($displayremark) {
            $currentremark = '';
            ...
            $remarklabeltext = get_string('criterionremark', 'gradingform_offline', $remarkinfo);

            if ($mode == gradingform_offline_controller::DISPLAY_EVAL) {
            ...
            } else if ($mode == gradingform_offline_controller::DISPLAY_EVAL_FROZEN) {
            ...
            }else if ($mode == gradingform_offline_controller::DISPLAY_REVIEW || $mode == gradingform_offline_controller::DISPLAY_VIEW) {
    ...
    public function level_template($mode, $options, $elementname = '{NAME}', $criterionid = '{CRITERION-id}', $level = null) {
        ...
        if ($mode == gradingform_offline_controller::DISPLAY_EDIT_FULL) {
            $definitionparams = array(
                ...
                'aria-label' => get_string('leveldefinition', 'gradingform_offline', $levelindex),
                'cols' => '10', 'rows' => '4'
            );
            ...
            $scoreparams = array(
                ...
                'aria-label' => get_string('scoreinputforlevel', 'gradingform_offline', $levelindex),
                'size' => '3',
                'value' => $level['score']
            );
        ...
        if ($mode == gradingform_offline_controller::DISPLAY_EVAL_FROZEN && $level['checked']) {
        ...
        if ($mode != gradingform_offline_controller::DISPLAY_EDIT_FULL &&
            ...
            $tdattributes['aria-label'] = get_string('level', 'gradingform_offline', $levelinfo);

            if ($mode != gradingform_offline_controller::DISPLAY_PREVIEW &&
                $mode != gradingform_offline_controller::DISPLAY_PREVIEW_GRADED) {
        ...
        $displayscore = true;
        if (!$options['showscoreteacher'] && in_array($mode, array(gradingform_offline_controller::DISPLAY_EVAL, gradingform_offline_controller::DISPLAY_EVAL_FROZEN, gradingform_offline_controller::DISPLAY_REVIEW))) {
            $displayscore = false;
        }
        if (!$options['showscorestudent'] && in_array($mode, array(gradingform_offline_controller::DISPLAY_VIEW, gradingform_offline_controller::DISPLAY_PREVIEW_GRADED))) {
            $displayscore = false;
        }
        if ($displayscore) {
            $scoreclass = 'score';
            ...
            $leveltemplate .= html_writer::tag('div', get_string('scorepostfix', 'gradingform_offline', $score), array('class' => $scoreclass));
        }
        if ($mode == gradingform_offline_controller::DISPLAY_EDIT_FULL) {
            $value = get_string('leveldelete', 'gradingform_offline', $levelindex);
            $buttonparams = array(
                'type' => 'submit',
    ...
    protected function rubric_template($mode, $options, $elementname, $criteriastr) {
        ...
        switch ($mode) {
            case gradingform_offline_controller::DISPLAY_EDIT_FULL:
                $classsuffix = ' editor editable'; break;
            case gradingform_offline_controller::DISPLAY_EDIT_FROZEN:
                $classsuffix = ' editor frozen';  break;
            case gradingform_offline_controller::DISPLAY_PREVIEW:
            case gradingform_offline_controller::DISPLAY_PREVIEW_GRADED:
                $classsuffix = ' editor preview';  break;
            case gradingform_offline_controller::DISPLAY_EVAL:
                $classsuffix = ' evaluate editable'; break;
            case gradingform_offline_controller::DISPLAY_EVAL_FROZEN:
                $classsuffix = ' evaluate frozen';  break;
            case gradingform_offline_controller::DISPLAY_REVIEW:
                $classsuffix = ' review';  break;
            case gradingform_offline_controller::DISPLAY_VIEW:
                $classsuffix = ' view';  break;
        }

        $rubrictemplate = html_writer::start_tag('div', array('id' => 'rubric-{NAME}', 'class' => 'clearfix gradingform_offline'.$classsuffix));

        // Rubric table.
        $rubrictableparams = array(
            'class' => 'criteria',
            'id' => '{NAME}-criteria',
            'aria-label' => get_string('rubric', 'gradingform_offline'));
        $rubrictable = html_writer::tag('table', $criteriastr, $rubrictableparams);
        $rubrictemplate .= $rubrictable;
        if ($mode == gradingform_offline_controller::DISPLAY_EDIT_FULL) {
            $value = get_string('addcriterion', 'gradingform_offline');
    ...
    protected function rubric_edit_options($mode, $options) {
        if ($mode != gradingform_offline_controller::DISPLAY_EDIT_FULL
                && $mode != gradingform_offline_controller::DISPLAY_EDIT_FROZEN
                && $mode != gradingform_offline_controller::DISPLAY_PREVIEW) {
            // Options are displayed only for people who can manage
            return;
        }
        $html = html_writer::start_tag('div', array('class' => 'options'));
        $html .= html_writer::tag('div', get_string('rubricoptions', 'gradingform_offline'), array('class' => 'optionsheading'));
        ...
        foreach ($options as $option => $value) {
            ...
            switch ($option) {
                case 'sortlevelsasc':
                    ...
                    $html .= html_writer::label(get_string($option, 'gradingform_offline'), $attrs['id'], false);
                    $value = (int)(!!$value); // make sure $value is either 0 or 1
                    if ($mode == gradingform_offline_controller::DISPLAY_EDIT_FULL) {
                        $selectoptions = array(0 => get_string($option.'0', 'gradingform_offline'), 1 => get_string($option.'1', 'gradingform_offline'));
                        ...
                        $html .= html_writer::tag('span', get_string($option.$value, 'gradingform_offline'), array('class' => 'value'));
                        if ($mode == gradingform_offline_controller::DISPLAY_EDIT_FROZEN) {
                    ...
                    break;
                default:
                    if ($mode == gradingform_offline_controller::DISPLAY_EDIT_FROZEN && $value) {
                    ...
                    if ($mode == gradingform_offline_controller::DISPLAY_EDIT_FROZEN || $mode == gradingform_offline_controller::DISPLAY_PREVIEW) {
                    ...
                    $html .= html_writer::tag('label', get_string($option, 'gradingform_offline'), array('for' => $attrs['id']));
                    break;
            }
            if (get_string_manager()->string_exists($option.'_help', 'gradingform_offline')) {
                $html .= $this->help_icon($option, 'gradingform_offline');
            }
    ...
    public function display_rubric($criteria, $options, $mode, $elementname = null, $values = null) {
        ...
        foreach ($criteria as $id => $criterion) {
            ...
            foreach ($criterion['levels'] as $levelid => $level) {
                ...
                if ($level['checked'] && ($mode == gradingform_offline_controller::DISPLAY_EVAL_FROZEN || $mode == gradingform_offline_controller::DISPLAY_REVIEW || $mode == gradingform_offline_controller::DISPLAY_VIEW)) {
                    $level['class'] .= ' checked';
     ...
     *
     * @param array $instances array of objects of type gradingform_offline_instance
     ...
     *
     * @param gradingform_offline_instance $instance
     ...
     */
    public function display_instance(gradingform_offline_instance $instance, $idx, $cangrade) {
        ...
        if ($cangrade) {
            ...
            $mode = gradingform_offline_controller::DISPLAY_VIEW;
            $showdescription = $options['showdescriptionstudent'];
        }
        $output = '';
        if ($showdescription) {
            $output .= $this->box($instance->get_controller()->get_formatted_description(), 'gradingform_offline-description');
        }
    ...
    public function display_regrade_confirmation($elementname, $changelevel, $value) {
        $html = html_writer::start_tag('div', array('class' => 'gradingform_offline-regrade', 'role' => 'alert'));
        if ($changelevel<=2) {
            $html .= html_writer::label(get_string('regrademessage1', 'gradingform_offline'), 'menu' . $elementname . 'regrade');
            $selectoptions = array(
                0 => get_string('regradeoption0', 'gradingform_offline'),
                1 => get_string('regradeoption1', 'gradingform_offline')
            );
            $html .= html_writer::select($selectoptions, $elementname.'[regrade]', $value, false);
        } else {
            $html .= get_string('regrademessage5', 'gradingform_offline');
    ...
    public function display_rubric_mapping_explained($scores) {
        ...
        if ($scores['minscore'] <> 0) {
            $html .= $this->output->notification(get_string('zerolevelsabsent', 'gradingform_offline'), 'error');
        }
        $html .= $this->output->notification(get_string('rubricmappingexplained', 'gradingform_offline', (object)$scores), 'info');
        return $html;
    }
